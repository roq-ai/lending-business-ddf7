...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['business-owner', 'loan-officer', 'loan-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<LenderInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/lenders` : null),
    () => getLenders({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Lender',
      path: isTenantUser ? `/lenders/view/${data?.[0]?.id}` : '/lenders',
      entity: 'lender',
      service: AccessServiceEnum.PROJECT,
      icon: FiBriefcase,
    },
    { name: 'Loan', path: '/loans', entity: 'loan', service: AccessServiceEnum.PROJECT, icon: FiDollarSign },
